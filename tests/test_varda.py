...
#: Every permissible value bound to an ontology term, and the term. Written
#: out rather than derived, because the point of the test is that adding a
#: mapping is deliberate: a `meaning:` asserts that two terms denote one
#: concept, and the obvious mappings in this vocabulary are false — the
#: profile's enum header lists which and why.
ONTOLOGY_TERMS = {
    ("TableRole", "FACT"): "qb:DataSet",
    ("ColumnRole", "MEASURE"): "qb:MeasureProperty",
    ("ColumnRole", "ATTRIBUTE"): "qb4o:LevelAttribute",
    ("ColumnRole", "DEGENERATE_DIMENSION"): "qb:DimensionProperty",
    ("ColumnRole", "VERSION_START"): "time:hasBeginning",
    ("ColumnRole", "VERSION_END"): "time:hasEnd",
}


def test_the_vocabulary_maps_exactly_these_terms() -> None:
    """A tripwire on the mappings, in both directions.

    A `meaning:` that disappears takes a claim out of the published
    vocabulary silently. One that appears is a new assertion that two terms
    denote the same concept, and this vocabulary is surrounded by plausible
    mappings that are wrong — `qb:AttributeProperty` qualifies an
    observation and not a dimension column, `qb:DimensionProperty` is a
    component of a cube and not a dimension table. Both directions are
    checked so neither happens by accident.
    """
    view = registry.varda_extension().profile_view
    assert view is not None
    found = {
        (enum, value): str(pv.meaning)
        for enum, spec in view.schema.enums.items()
        for value, pv in (spec.permissible_values or {}).items()
        if pv.meaning
    }
    assert found == ONTOLOGY_TERMS


def test_every_mapped_prefix_is_declared() -> None:
    """A CURIE whose prefix is undeclared expands to nothing.

    `meaning:` is stored as written, so a typo in the prefix survives load
    and reaches the reference page and any RDF built from the vocabulary as
    a term nobody can resolve.
    """
    view = registry.varda_extension().profile_view
    assert view is not None
    declared = set(view.schema.prefixes)
    for term in ONTOLOGY_TERMS.values():
        assert term.split(":", 1)[0] in declared, term


def test_the_reference_page_publishes_the_mappings() -> None:
    """The mapping is worth having because a reader sees it.

    It reaches no model's RDF — a domain model does not import the
    vocabulary, so `varda:role: MEASURE` travels as a string and the enum is
    never referenced. What the mapping buys is the generated reference
    saying which standard concept a Varda word denotes.
    """
    page = ROOT / "docs" / "reference" / "vocabulary.md"
    if not page.exists():
        pytest.skip("reference pages are generated by scripts/gen_reference.py")
    text = page.read_text(encoding="utf-8")
    for term in ONTOLOGY_TERMS.values():
        assert f"Ontology term: `{term}`" in text
